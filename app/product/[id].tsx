  ...
  const handleAddToCart = () => {
    if (!selectedSize) {
      Toast.show({
        ...
        text2: 'Select a size before adding to cart'
      })
      return
    }
      ...
      <View className='flex-1 bg-white'>
        <ScrollView contentContainerStyle={{ paddingBottom: 100 }}>
          ...
          <View className='relative-h-[450px] bg-gray-100 mb-6'>
            ...
            <View className='absolute top-12 left-4 right-4 flex-row justify-between items-center'>
              <TouchableOpacity
                onPress={() => router.back()}
                className='w-10 h-10 py-1 bg-white/80 rounded-full items-center'
              >
                <Ionicons name='arrow-back' size={24} color={COLORS.primary} />
              </TouchableOpacity>
              <TouchableOpacity
                onPress={() => toggleWishlist(product)}
                className='w-10 h-10 py-1 mb-1 bg-white/80 rounded-full items-center'
              >
                <Ionicons
          ...
          <View className='px-5'>
            ...
            <View className='flex-row justify-between items-start mb-2'>
              <Text className='text-2x1 font-bold text-primary flex-1 mr-4'>
                {product.name}
              </Text>
              <View className='flex-row justify-between items-start mb-2'>
                ...
                <View className='flex-row gap-3 mb-6 flex-wrap'>
                  {product.sizes.map(size => (
                    <TouchableOpacity
                      ...
                      className={`w-12 h-12 rounded-full items-center border ${
                        selectedSize === size
        ...
        <View className=' absolute bottom-0 left-0 flex-row right-0 p-4 bg-white border-t border-gray-100 '>
          ...
          >
            ...
            <View className='absolute-top-2 right-4 size-4 z-10 bg-black rounded-full justify-center'>
              <Text className='text-[9px]'>{itemCount}</Text>
            </View>
          </TouchableOpacity>
        </View>
      </View>
